...
class NukiLocal extends utils.Adapter {
  ...
  async onReady() {
    ...
    try {
      ...
      this.log.info(
        "Nuki Local adapter started"
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.log.error(
        `Could not start adapter: ${message}`
      );
      await this.setStateAsync(
        "info.connection",
        false,
  ...
  async startMqttBroker() {
    ...
    const username = String(
      (_a = this.config.mqttUsername) != null ? _a : "nuki"
    );
    const password = String(
      (_b = this.config.mqttPassword) != null ? _b : ""
    );
    const instanceDataDir = utils.getAbsoluteInstanceDataDir(
      this
    );
    const mqttDataDir = (0, import_node_path.join)(
      instanceDataDir,
      "mqtt-leveldb"
    );
    ...
    this.log.info(
      `MQTT persistence directory: ${mqttDataDir}`
    );
    this.mqttDatabase = new import_level.Level(
      mqttDataDir
    );
    this.mqttPersistence = aedesPersistenceLevel(
      this.mqttDatabase
    );
    ...
    this.mqttBroker.authenticate = (client, mqttUsername, mqttPassword, callback) => {
      ...
      if (valid) {
        this.log.info(
          `MQTT client authenticated: ${(_b2 = client == null ? void 0 : client.id) != null ? _b2 : "unknown"}`
        );
      } else {
        this.log.warn(
          `MQTT authentication rejected for client: ${(_c = client == null ? void 0 : client.id) != null ? _c : "unknown"}`
        );
      }
      callback(
        null,
    ...
    this.mqttBroker.on(
      ...
      (packet, client) => {
        ...
        this.log.info(
          `Nuki MQTT: ${packet.topic} = ${value}`
        );
        void this.handleNukiMessage(
          packet.topic,
          value
        );
      }
    );
    this.mqttServer = (0, import_node_net.createServer)(
    ...
    await new Promise(
      (resolve, reject) => {
        ...
        const onError = (error) => {
          var _a2;
          (_a2 = this.mqttServer) == null ? void 0 : _a2.off(
            "listening",
            onListening
          );
          reject(error);
    ...
    this.log.info(
      `Integrated MQTT broker listening on port ${port}`
    );
  }
  async restoreRetainedNukiStates() {
    ...
    this.log.info(
      "Restoring retained Nuki MQTT states..."
    );
    let restored = 0;
    const stream = this.mqttPersistence.createRetainedStreamCombi(
      ["nuki/#"]
    );
    for await (const packet of stream) {
      ...
      const payload = Buffer.isBuffer(
        packet.payload
      ) ? packet.payload.toString() : String(
        (_a = packet.payload) != null ? _a : ""
      );
      await this.handleNukiMessage(
        packet.topic,
        payload
      );
      restored++;
    ...
    this.log.info(
      `Restored ${restored} retained Nuki MQTT state(s)`
    );
  }
    ...
    try {
      ...
      switch (property) {
        ...
        case "batteryChargeState":
          await this.setNumberState(
            `${deviceId}.battery.percent`,
            "Battery",
            payload,
            "%",
            "value.battery"
        ...
        case "keypadBatteryCritical":
          ...
          break;
        case "state":
          await this.handleLockState(
            deviceId,
            payload
          );
          break;
        case "doorsensorState":
          await this.handleDoorState(
            deviceId,
            payload
          );
          break;
        case "lockActionEvent":
          await this.handleLockActionEvent(
            deviceId,
            payload
          );
          break;
        case "commandResponse":
          await this.handleCommandResponse(
            deviceId,
            payload
          );
          break;
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.log.error(
        `Could not process Nuki MQTT message ${topic}: ${message}`
      );
    }
  }
  /*
  ...
  async ensureDevice(deviceId) {
    ...
    const existingPromise = this.initializingDevices.get(
      normalized
    );
    if (existingPromise) {
    ...
    const initialization = this.initializeDevice(
      deviceId,
      normalized
    );
  ...
  async initializeDevice(deviceId, normalized) {
    ...
    await this.ensureKnownDeviceObjects(
      deviceId
    );
  ...
  async ensureKnownDeviceObjects(deviceId) {
    await this.ensureMappedStateObject(
    ...
    await this.ensureStateObject(
      `${deviceId}.advanced.smartlockId`,
      "Web API Smartlock ID",
      "number",
      "value"
    );
    await this.ensureStateObject(
      `${deviceId}.advanced.serverState`,
      "Nuki server state",
      "number",
      "value"
    );
    ...
    await this.createCommandState(
      `${deviceId}.commands.lock`,
      "Lock"
    );
    await this.createCommandState(
      `${deviceId}.commands.unlock`,
      "Unlock"
    );
    await this.createCommandState(
      `${deviceId}.commands.unlatch`,
      "Unlatch"
    );
    await this.createCommandState(
      `${deviceId}.commands.lockNgo`,
      "Lock'n'Go"
    );
    await this.createCommandState(
      `${deviceId}.commands.lockNgoUnlatch`,
      "Lock'n'Go with unlatch"
    );
    await this.createCommandState(
      `${deviceId}.commands.fullLock`,
      "Full lock"
    );
    await this.ensureStateObject(
      `${deviceId}.commands.lastResult`,
      "Last command result",
      "number",
      "value"
    );
  ...
  async cleanupLegacyObjects(deviceId) {
    ...
    for (const id of legacy) {
      const object = await this.getObjectAsync(
        id
      );
      if (!object) {
        continue;
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        this.log.warn(
          `Could not remove old object ${id}: ${message}`
        );
      }
    }
  }
  ...
  async updateDeviceIcon(deviceId) {
    const chargingState = await this.getStateAsync(
      `${deviceId}.battery.charging`
    );
    const doorOpenState = await this.getStateAsync(
      `${deviceId}.status.doorOpen`
    );
    const lockState = await this.getStateAsync(
      `${deviceId}.status.lockState`
    );
    ...
    const lockStateValue = typeof (lockState == null ? void 0 : lockState.val) === "number" ? lockState.val : Number(
      lockState == null ? void 0 : lockState.val
    );
  ...
  async handleLockActionEvent(deviceId, payload) {
    ...
    if (parts.length < 5) {
      this.log.warn(
        `Invalid lockActionEvent: ${payload}`
      );
      return;
    }
    ...
    const configuredKeypadUser = codeId > 0 ? this.findConfiguredKeypadUser(
      codeId
    ) : void 0;
    const resolvedUserName = this.findAuthorizationName(
      deviceId,
      authId
    );
  ...
  findAuthorizationName(deviceId, authId) {
    var _a;
    const smartlockId = this.webSmartlockIds.get(
    ...
    return (_a = this.authorizationNames.get(
      smartlockId
    )) == null ? void 0 : _a.get(
      authId
    );
  ...
  async onStateChange(id, state) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.log.error(
        `Could not execute command ${command}: ${message}`
      );
      await this.setStateAsync(
        relativeId,
        false,
  ...
  async publishNukiCommand(topic, payload) {
    ...
    this.log.info(
      `Nuki command published: ${topic} = ${payload}`
    );
  }
  ...
  async startWebApi() {
    var _a;
    const token = String(
      (_a = this.config.webApiToken) != null ? _a : ""
    ).trim();
    if (!token) {
      this.log.warn(
        "Nuki Web API is enabled but no API token is configured"
      );
      return;
    }
    const intervalSeconds = Math.max(
      60,
      Number(
        this.config.webApiInterval
      ) || 300
    );
    this.log.info(
      `Nuki Web API enabled, update interval: ${intervalSeconds} seconds`
    );
    try {
      ...
      () => {
        void this.updateNukiWebData().catch(
          (error) => {
            const message = error instanceof Error ? error.message : String(error);
            this.log.warn(
              `Nuki Web API update failed: ${message}`
            );
          }
        );
      },
  ...
  async updateNukiWebData() {
    const devices = await this.nukiWebRequest(
      "/smartlock"
    );
    if (!Array.isArray(
      devices
      ...
      throw new Error(
        "Invalid response from /smartlock"
      );
    }
    let authorizations = [];
    try {
      const result = await this.nukiWebRequest(
        "/smartlock/auth"
      );
      if (Array.isArray(
        result
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.log.warn(
        `Could not load Nuki authorizations: ${message}`
      );
    }
    for (const device of devices) {
      const smartlockId = Number(
        device.smartlockId
      );
      if (!Number.isFinite(
        smartlockId
      ...
      const conversion = this.getMqttDeviceIdFromWebDevice(
        device
      );
      ...
      await this.storeWebDeviceData(
        deviceId,
        device
      );
      const deviceAuths = authorizations.filter(
        (auth) => Number(
          auth.smartlockId
        ) === smartlockId
      );
      await this.storeWebAuthorizations(
        deviceId,
        smartlockId,
        deviceAuths
      );
      try {
        const logs = await this.nukiWebRequest(
          `/smartlock/${smartlockId}/log?limit=20`
        );
        if (Array.isArray(
          logs
        )) {
          await this.storeWebLogs(
            deviceId,
            logs
          );
        }
        ...
        const message = error instanceof Error ? error.message : String(error);
        this.log.warn(
          `Could not load activity logs for ${deviceId}: ${message}`
        );
      }
    }
  }
  async nukiWebRequest(path) {
    ...
    if (!token) {
      throw new Error(
        "Nuki Web API token missing"
      );
    }
    const response = await fetch(
      `https://api.nuki.io${path}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `HTTP ${response.status} ${response.statusText}` + (body ? `: ${body.substring(
          0,
          300
        )}` : "")
      );
  ...
  getMqttDeviceIdFromWebDevice(device) {
    try {
      const originalHex = BigInt(
        device.smartlockId
      ).toString(16).toUpperCase();
      const deviceId = originalHex.slice(-8).padStart(
        8,
        "0"
      );
      return {
  ...
  normalizeDeviceId(deviceId) {
    const normalized = deviceId.trim().toUpperCase().replace(
      /^0X/,
      ""
    );
    return normalized.slice(-8).padStart(
      8,
      "0"
    );
  }
  async storeWebDeviceData(deviceId, device) {
    ...
    if (typeof state.doorsensorBatteryCritical === "boolean") {
      await this.setStateAsync(
        `${deviceId}.battery.doorSensorCritical`,
        state.doorsensorBatteryCritical,
        true
      );
    }
  ...
  async storeWebAuthorizations(deviceId, smartlockId, authorizations) {
    ...
    this.authorizationNames.set(
      smartlockId,
      authNames
    );
    const safe = authorizations.map(
      (auth) => ({
        id: auth.id,
      ...
      try {
        ...
        if (this.mqttServer) {
          await new Promise(
            (resolve) => {
              var _a;
              (_a = this.mqttServer) == null ? void 0 : _a.close(
                () => resolve()
              );
            }
          );
        ...
        if (this.mqttBroker) {
          await this.mqttBroker.close();
          this.mqttBroker = null;
        }
        this.mqttPersistence = null;
        this.mqttDatabase = null;
        this.log.info(
          "Nuki Local adapter stopped"
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        this.log.warn(
          `Error while stopping adapter: ${message}`
        );
      } finally {
        callback();
      }
...
if (require.main !== module) {
  module.exports = (options) => new NukiLocal(
    options
  );
} else {
  (() => new NukiLocal())();
